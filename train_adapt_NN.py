...
class ASR(sb.Brain):
    ...
    def attack_adapt_CW(
        ...
        for batch in tqdm(train_set, dynamic_ncols=True, disable=not progressbar):
            ...
            for i in batch.path:
                if (not os.path.exists(i.replace(hparams["path_adv"], hparams["path_NN"]))):
    ...
    def attack_1st_stage(self, batch, hparams):
        ...
        model_NN = Deep().to(self.device)
        model_NN.load_state_dict(torch.load("NN-model-uai.pth")) # model_NN.load_state_dict(torch.load("NN-model.pth"))
        model_NN.eval()
        for iter_1st_stage_idx in range(self.max_iter_1):
            ...
            for local_batch_size_idx in range(local_batch_size):
                ...
                torchaudio.save("adv_ex_15.flac", almost_successful[local_batch_size_idx][ :real_lengths[local_batch_size_idx]].cpu()[None, :], self.sample_rate)
                data_adv, sample_rate = torchaudio.load("adv_ex_15.flac")
            ...
            if iter_1st_stage_idx == self.max_iter_1 - 1:
                ...
                for (local_batch_size_idx, dirct) in enumerate(batch.path):
                    dirct = dirct.replace(hparams["path_adv"], hparams["path_NN"])
                # for local_batch_size_idx in range(local_batch_size):
                    if successful_adv_input_2[local_batch_size_idx] is None:
                        successful_adv_input_2[local_batch_size_idx] = masked_adv_input[local_batch_size_idx]
                        # trans[local_batch_size_idx] = decoded_output[local_batch_size_idx]
                        with open(hparams["unsuccesfull_adv_NN"], 'a') as myfile:
                            wr = csv.writer(myfile)                            
                            wr.writerow([[dirct], [first_hit[local_batch_size_idx]], [best_hit[local_batch_size_idx]], 
                                [best_eta[local_batch_size_idx]], [count_succs[local_batch_size_idx]], [self.alpha], [characteristic]])
                            myfile.close()
                    else:
                        with open(hparams["succesfull_adv_NN"], 'a') as myfile:
                            wr = csv.writer(myfile)
                            wr.writerow([[dirct], [first_hit[local_batch_size_idx]], [best_hit[local_batch_size_idx]], 
                                [best_eta[local_batch_size_idx][0]], [count_succs[local_batch_size_idx]], [self.alpha], [best_score[local_batch_size_idx]]])
                            myfile.close()
                    # print(dirct)
    ...
    def forward_1st_stage(
        ...
        NN_input = self.characteristics(p_ctc_prob)
        NN_input =  torch.stack(NN_input)

        y_pred_2 = model(NN_input)
        # print("y_pred_2: ", y_pred_2)
        relu = torch.nn.ReLU()
        loss_2 = relu(torch.abs(torch.tensor(1.0) - y_pred_2)) # * 10
        loss = (1-self.alpha) * loss_1 + self.alpha * loss_2

        return loss, loss_2, y_pred_2.item(), local_delta, masked_adv_input, local_delta_rescale

    def characteristics(self, p_ctc_prob):
        score_list = []
        # EM = 5
        char_1_prob = -1 * torch.sum(p_ctc_prob * torch.log(p_ctc_prob), axis=1)
        score_list.append(torch.mean(char_1_prob))
        score_list.append(torch.max(char_1_prob))
        score_list.append(torch.min(char_1_prob))
        score_list.append(torch.median(char_1_prob))

        values_max, _ = torch.max(p_ctc_prob, axis=1)
        score_list.append(torch.mean(values_max))
        score_list.append(torch.max(values_max))
        score_list.append(torch.min(values_max))
        score_list.append(torch.median(values_max))

        values_min, _ = torch.min(p_ctc_prob, axis=1)
        values_min = torch.log(values_min)
        score_list.append(torch.mean(values_min))
        score_list.append(torch.max(values_min))
        score_list.append(torch.min(values_min))
        score_list.append(torch.median(values_min))

        values, indx = torch.median(p_ctc_prob, axis=1)
        values_median = torch.log(values)
        score_list.append(torch.mean(values_median))
        score_list.append(torch.max(values_median))
        score_list.append(torch.min(values_median))
        score_list.append(torch.median(values_median))

        # p_ctc_prob = -1 * p_ctc_prob * torch.log(p_ctc_prob)
        # characteristic = torch.median(torch.sum(p_ctc_prob, axis=1))
        # print(characteristic)
        
        d_sym, kl_sym = [], []
        for i in range(p_ctc_prob.shape[0] - 1):
            m_2 = 0.5 * (p_ctc_prob[i, :] + p_ctc_prob[i + 1, :])
            left = 0.5 * F.kl_div(torch.log(m_2), p_ctc_prob[i, :], reduction = "sum")
            right = 0.5 * F.kl_div(torch.log(m_2), p_ctc_prob[i + 1, :], reduction = "sum")
            entr = F.kl_div(torch.log(p_ctc_prob[i + 1, :]), p_ctc_prob[i, :], reduction = "sum")
            jsd = left + right
            if torch.isinf(jsd) or jsd == 0 or torch.isinf(entr) or entr==0:
                print("ERROR")
            else:
                d_sym.append(jsd.unsqueeze(0))
                kl_sym.append(entr.unsqueeze(0))
        jsd = torch.cat(d_sym)
        kld = torch.cat(kl_sym)
        
        score_list.append(torch.mean(jsd))
        score_list.append(torch.max(jsd))
        score_list.append(torch.min(jsd))
        score_list.append(torch.median(jsd))

        score_list.append(torch.mean(kld))
        score_list.append(torch.max(kld))
        score_list.append(torch.min(kld))
        score_list.append(torch.median(kld))
...
if __name__ == "__main__":
    ...
    '''
    adapt_test = dataio_prepare_2(hparams, hparams["adaptive_transcriptions_2"])
    asr_brain.hparams.wer_file = hparams["adapt_file"]
    # Testing
    asr_brain.evaluate(
        adapt_test,
